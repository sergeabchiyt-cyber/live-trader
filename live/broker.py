...
TestnetBroker / LiveBroker — mirror strategy events to Binance Spot
              (testnet.binance.vision / api.binance.com) using LIMIT entries and
              a stop-limit SL + LIMIT TP pair, re-armed whenever the trail moves
              the stop. NOTE: this sandbox is geo-blocked from Binance (HTTP 451)
              so these are shipped as reference code — always validate on
              testnet from your own machine before enabling live.
...
class BinanceBroker:
    """Mirrors strategy events onto Binance Spot. Falls back to a logged status
    on any error and always keeps the paper record as the PnL source of truth."""
        ...
        try:
            ...
            if m.get("sl"):
                self.cli.place_stop_limit(self.cfg.symbol, close_side, m["qty"],
                                          m["sl"], m["sl"])
            if m.get("tp"):
                self.cli.place_limit(self.cfg.symbol, close_side, m["qty"], m["tp"])
            m["armed"] = True
    ...
    def on_open(self, ev):
        q = round(self.cfg.position_usd / ev["entry"], 6)
        if self._tradable():
            try:
